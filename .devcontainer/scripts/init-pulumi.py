...
response = subprocess.run(
    ["gum", "confirm", "do you want to continue? (y/n)"],
...
for key, value in data["config"].items():
    ...
    if "secure" in value:
        ...
        new_secure_value = "".join(
            random.choices(string.ascii_uppercase + string.digits, k=16)
        )
        print(
            f"would you like to update the secure value for {key}? leave blank for {new_secure_value}"
        )
        ...
        subprocess.run(
            [
                ...
                "--path",
                f"{key}",
                f"{data['config'][key]['secure']}",
                "--cwd=/workspace/infra/fridge",
                "--config-file=/workspace/infra/fridge/Pulumi.dev.yaml",
            ]
        )
    # if value is an array
    elif isinstance(value, list):
        print(
            f"would you like to update the array value for {key} (as comma-separated list)? (current value: {value})"
        )
    ...
    # if value is a string
        ...
        if response:
            ...
            print(
                f"pulumi config set {key} {response} --cwd=/workspace/infra/fridge --config-file=/workspace/infra/fridge/Pulumi.dev.yaml"
            )
            subprocess.run(
                [
                    "pulumi",
