...
: Representatives from the technical groups running FRIDGE, information governance, and researchers as appropriate to manage risk in day-to-day operations. See [details](#role-operational-management-group) and [FRIDGE Governance](shared_gov_model.md).
...
: Leads research projects, submits Safe Project applications, and nominates researchers. See [details](#role-principal-investigator).

Safe Researcher
: Researcher who has completed training, signed attestation, and been approved for data access. Authorised to use the TRE. See [details](#role-safe-researcher).

Job Submitter
: A subset of {term}`Safe Researcher` who can submit FRIDGE jobs (i.e. dispatch jobs to a remote resource). See [details](#role-job-submitter).
...
- Operates the full stack TRE (the "front door" TRE) and the FRIDGE TRE ({term}`Satellite TRE`).
...
- Deploys and tears down the Access Cluster and Isolated Cluster on request from {term}`TRE Administrator`s.
- Applies lockdown configurations to clusters once instructed by {term}`TRE Administrator`s.
- Provides connection details to {term}`TRE Administrator`s following deployment.
...
- Receives requirements from data providers and sets requirements that flow down to the {term}`Operational Management Group`.
...
- Reports risk and performance information upward to {term}`FRIDGE Federation Governance`.
- Comprises the {term}`Information Governance Team`, {term}`Hosting Administrator`, {term}`TRE Administrator`, and {term}`Principal Investigator`s / {term}`Safe Researcher`s as appropriate.
...
- Requests FRIDGE deployment from {term}`Hosting Administrator`s, providing configuration requirements.
- Requests lockdown from {term}`Hosting Administrator`s and confirms lockdown is complete before reporting back to the {term}`TRE Operator Organisation`.
- Initiates TRE disconnection from FRIDGE at teardown.
- Requests FRIDGE teardown from {term}`Hosting Administrator`s.
...
- Deploys the Access Cluster and Isolated Cluster on request from {term}`TRE Administrator`s.
- Provides connection details to {term}`TRE Administrator`s following deployment.
- Applies network and cluster lockdown configurations on instruction from {term}`TRE Administrator`s.
...
- Nominates {term}`Safe Researcher`s for data access.
...
A researcher who has been formally accredited to access sensitive data within the TRE. {term}`Safe Researcher`s work within the TRE but do not directly interact with the FRIDGE infrastructure.
...
- Submits job specifications to the FRIDGE API on behalf of {term}`Safe Researcher`s.
- Monitors job status and downloads results from the FRIDGE API.
- Shares results with {term}`Safe Researcher`s within the TRE.
