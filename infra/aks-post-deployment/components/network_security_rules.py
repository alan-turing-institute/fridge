    ...
    def __init__(
        ...
        isolated_cluster_nsg_rules = [
            ...
            network.SecurityRuleArgs(
                ...
                source_address_prefix=isolated_vnet_cidr,
                destination_address_prefix=isolated_vnet_cidr,
                description="Allow all traffic from isolated cluster VNet",
            ),
            network.SecurityRuleArgs(
            ...
            network.SecurityRuleArgs(
                name="AllowOutboundToIsolatedNodes",
                priority=350,
                direction=network.SecurityRuleDirection.OUTBOUND,
                access=network.SecurityRuleAccess.ALLOW,
                protocol=network.SecurityRuleProtocol.ASTERISK,
                ...
                source_address_prefix=isolated_vnet_cidr,
                destination_address_prefix=isolated_vnet_cidr,
                description="Allow outbound traffic to isolated cluster nodes (for API Proxy and FRIDGE API access)",
            ),
            network.SecurityRuleArgs(
